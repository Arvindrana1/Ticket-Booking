...
public class TrainService {
    ...
    private File getTrainFile() {
        File f = new File(TRAIN_DB_PATH);
        if (f.exists()) {
            return f;
        }
        String fallback = TRAIN_DB_PATH.substring(TRAIN_DB_PATH.indexOf("/") + 1);
        return new File(fallback);
    }

    public TrainService() throws IOException {
        trainList = objectMapper.readValue(getTrainFile(), new TypeReference<List<Train>>() {
        });
    }
    ...
    private void saveTrainListToFile() {
        try {
            objectMapper.writeValue(getTrainFile(), trainList);
        } catch (IOException e) {
